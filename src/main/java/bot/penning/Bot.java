...
public class Bot {
	...
	public static void main(String[] args) {
		 ...
		 * We have to hard code our list of command files since iterating over a list of
		 * files in a resource directory
		 * is overly complicated for such a simple demo and requires handling for both
		 * IDE and .jar packaging.
...
// event.getMessage().getChannel().block().createMessage("You have written " +
// writerIndex.get(event.getMember()).getProgress() + " of " +
// writerIndex.get(event.getMember()).getGoal() + " " +
// writerIndex.get(event.getMember()).getGoalType()).block();
// }
// else {
// event.getMessage().getChannel().block().createMessage("Create a goal
// first!").block();
// }
//
// });
...
//
//
// //If the writerIndex map contains an object with the key of the user who
// called the command,
// //then the object is completely cleared.
// //this makes it so creating a new goal replaces the last one, and a person
// can not have 2 goals at once.
// if (writerIndex.containsKey(event.getMember())) {
...
// // System.out.println("String content = event.getMessage().getContent(): " +
// content);
// String[] commandContent = content.split(" ");
// if (commandContent.length >= 3) {
// Goal writerGoal = new Goal(Integer.parseInt(commandContent[1]),
// commandContent[2]);
// event.getMessage().getChannel().block().createMessage("Goal of " +
// writerGoal.getGoal() + " " + writerGoal.getGoalType()
// + " set!").block();
//
// writerIndex.put(event.getMember(), writerGoal);
//
// System.out.println("Created goal: " + writerGoal.getGoal() + " " +
// writerGoal.getGoalType());
//
//
// }
// else {
...
// if (writerIndex.containsKey(event.getMember())) {
// writerIndex.remove(event.getMember());
// event.getMessage().getChannel().block().createMessage("Goal has been
// reset!").block();
// } else {
// event.getMessage().getChannel().block().createMessage("There is no goal to
// reset! Create a goal!").block();
// }
//
// });
//
...
//
// event.getMessage().getChannel().block().createMessage("Progress updated! You
// have written " + writerIndex.get(event.getMember()).getProgress() + " " +
// writerIndex.get(event.getMember()).getGoalType() + " of " +
// writerIndex.get(event.getMember()).getGoal() + " " +
// writerIndex.get(event.getMember()).getGoalType()).block();
//
// if (writerIndex.get(event.getMember()).getProgress() /
// writerIndex.get(event.getMember()).getGoal() == 0) {
//
// }
//
//
...
//
// event.getMessage().getChannel().block().createMessage("Word skirmish: set
// amount of brief time (i.e. 10 minutes), try to write as many words as you
// can. \nWord battle: Much like a skirmish, but over a longer period of time
// (i.e. by the end of the day) \nWord war: a series of skirmishes over a period
// of time, i.e. hours or days. \nWord sprint: get to a set amount of words as
// quickly as possible.").block();
//
// });
//
...
//
// if (commandContent.length > 3) {
// event.getMessage().getChannel().block().createMessage("Invalid number of
// arguments. Try again!").block();
// return;
// }
//
...
//
// //stop user from creating a skirmish with length 0 (because calculating the
// average for total creates a divide by zero scenario)
// if (time == 0) {
// event.getMessage().getChannel().block().createMessage("Length of skirmish
// cannot be zero! Try again!");
// return;
// }
//
// //Let's user know the length is too long
// if (time > 60) {
// event.getMessage().getChannel().block().createMessage("Length is too long!
// Try starting a word battle instead.").block();
// return;
// }
//
...
//
// if (writerIndex.containsKey(event.getMember())) { //if user has created a
// goal; updates goal progress
// writerIndex.get(event.getMember()).addWords(totalWords);
// event.getMessage().getChannel().block().createMessage("You have written " +
// totalWords + " " + writerIndex.get(event.getMember()).getGoalType() + " for
// an average of " + averageWords + " " +
// writerIndex.get(event.getMember()).getGoalTypeAvg()).block();
// event.getMessage().getChannel().block().createMessage("Progress updated! You
// have written " + writerIndex.get(event.getMember()).getProgress() + " " +
// writerIndex.get(event.getMember()).getGoalType() + " of " +
// writerIndex.get(event.getMember()).getGoal() + " " +
...
// } else if (typeWritten != null) { //if user does NOT have goal, and adds
// !total with a keyword (i.e. "lines");
// event.getMessage().getChannel().block().createMessage("You have written " +
// totalWords + " " + typeWritten + " for an average of " + averageWords + " " +
// typeWritten + " per minute.").block();
// writersEntered.add(typeWritten);
// } else { //if user does NOT have goal, and does not specify with keyword;
// defaults to words
// event.getMessage().getChannel().block().createMessage("You have written " +
// totalWords + " words for an average of " + averageWords + " wpm.").block();
...
// event.getMessage().getChannel().block().createMessage("Skirmish is
// incomplete! Try again after it has finished.").block();
// }
//
//
...
// event.getMessage().getChannel().block().createMessage("You have written " +
// totalWords + " " + writerIndex.get(event.getMember()).getGoalType()).block();
// } else {}
//
//
...
//
// } */ else {
// event.getMessage().getChannel().block().createMessage(commandContent[1] + "
// not found. Please try with a valid war ID#.").block();
//
// }
//
// schedule.schedule(() -> {
...
// catch (Exception e) {
// e.printStackTrace();
// event.getMessage().getChannel().block().createMessage("Invalid
// command.").block();
// }
//
//
...
////
////
//// //stop user from creating a battle with length 0 (because calculating the
// average for total creates a divide by zero scenario)
//// //not that anyone would do that, so really this is just to stop *me* from
...
//// event.getMessage().getChannel().block().createMessage("Length is too short!
// Try starting a word skirmish instead.").block();
//// return;
//// }*/
////
...
// start = Integer.parseInt(commandContent[3]);
// System.out.println("goal: " + goal + " | time: " + time + " | start: " +
// start);
// }
// catch (Exception e) {
// e.printStackTrace();
// event.getMessage().getChannel().block().createMessage("Invalid sprint
// command. Try again!");
// }
//
...
// sprint.setGoal(goal);
//
// event.getMessage().getChannel().block().createMessage("Sprint #" +
// sprint.getIndex() + " created for " + time + " minutes, and will start in " +
// start + " minutes.").block();
//
//
...
// event.getMessage().getChannel().block().createMessage("Sprint #" +
// sprint.getIndex() + " starts now! Get to " + goal + " as quickly as you can!
// Enter `!total` when finished!").block();
//
...
// event.getMessage().getChannel().block().createMessage("Sprint #" +
// sprint.getIndex() + " ends now!").block();
// sprint.setComplete();
// } catch (InterruptedException e) {
// e.printStackTrace();
// }
//
// event.getMessage().getChannel().block().createMessage("Use `'!total " +
// sprint.getIndex() + " [amount written]'` to add your total.").block();
//
//
// warIndex++;
//
